...
import { createModelAdapter, ModelError, type StreamEvent } from '../server/services/modelAdapter.js';
...
describe('model adapter long SSE', () => {
  ...
  it('rejects upstream EOF without [DONE]', async () => { install([`data: ${JSON.stringify({ choices: [{ delta: { content: 'partial code' } }] })}\n\n`], 7); try { await assert.rejects(async () => { await collect(createModelAdapter({ modelId: 'test-model', baseUrl: 'https://example.invalid', apiKey: 'test' })); }, (error: unknown) => error instanceof ModelError && error.code === 'invalid_response'); } finally { globalThis.fetch = originalFetch; } });
