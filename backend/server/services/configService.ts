...
import type { AppLimitsConfig, AppSettingsConfig, SystemPromptConfig } from '../../types.js';
...
export async function getAppSettingsConfig(){ const now=Date.now(); if(settingsCache&&settingsCache.expires>now)return settingsCache.value; const remoteData=await remote('settings'); const v={...DEFAULT_SETTINGS,...(remoteData||persistentStorage.getAppSettingsConfig()||{})} as AppSettingsConfig; persistentStorage.saveAppSettingsConfig(v); settingsCache={value:v,expires:now+CONFIG_CACHE_TTL_MS}; return v; }
export async function updateAppSettingsConfig(newSettings:Partial<AppSettingsConfig>,updatedBy:string){ const c=await getAppSettingsConfig(); const v={...c,...newSettings,updatedAt:new Date().toISOString(),updatedBy}; persistentStorage.saveAppSettingsConfig(v); settingsCache={value:v,expires:Date.now()+CONFIG_CACHE_TTL_MS}; if(hasServiceAccount)await adminDb.collection('appConfig').doc('settings').set(v,{merge:true}); return v; }
